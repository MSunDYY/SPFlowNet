...
def main():
    ...
    global args 
    args = cmd_args.parse_args_from_yaml('./configs_with_occlusions/config_train_KITTI_r.yaml')
    ...
    # Creating Dir
    experiment_dir = Path('./experiment_occ/')
    experiment_dir.mkdir(exist_ok=True)
    file_dir = Path(str(experiment_dir) + '/%s-KITTI_r-'%args.exp_params['model_name'] + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
    ...
    os.system('cp %s %s' % ('train_KITTI_r.py', saved_dir))
    os.system('cp %s %s' % ('./configs_with_occlusions/config_evaluate_occ.yaml', saved_dir))
    os.system('cp %s %s' % ('./configs_with_occlusions/config_train_KITTI_r.yaml', saved_dir))
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset,
        batch_size=args.exp_params['val_batch_size'],
        shuffle=False,
    )
    ...
    for epoch in range(args.exp_params['epochs']):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            # print(loss, loss_consistency.sum())
